...
const handleGET = async (req: NextApiRequest, res: NextApiResponse, session: any) => {
  ...
  const where = box === 'sent' ? { senderId: userId } : { receiverId: userId };
...
const handlePOST = async (req: NextApiRequest, res: NextApiResponse, session: any, tm: any) => {
  ...
  // 같은 팀인지 확인
  const receiverTm = await prisma.teamMember.findFirst({
    where: { teamId: tm.teamId, userId: receiverId },
  });
  if (!receiverTm) {
    return res.status(400).json({ error: { message: '같은 회사 소속이 아닙니다.' } });
  }

  const message = await prisma.message.create({
    data: { senderId: session.user.id, receiverId, title, content },
    include: {
...
const handlePUT = async (req: NextApiRequest, res: NextApiResponse, session: any) => {
  ...
  if (!msg || msg.receiverId !== session.user.id) return res.status(403).json({ error: { message: 'Forbidden' } });
