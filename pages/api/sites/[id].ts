...
const handlePUT = async (id: string, req: NextApiRequest, res: NextApiResponse, session: any, tm: any) => {
  const {
    ...
    siteManager, settlementAmount, settlementNote,
    hasIssue, issueNote,
  ...
  const site = await prisma.site.update({
    ...
    data: {
      ...
      ...(siteManager !== undefined && { siteManager }),
      ...(settlementAmount !== undefined && { settlementAmount: settlementAmount ? Number(String(settlementAmount).replace(/,/g, '')) : null }),
      ...(settlementNote !== undefined && { settlementNote }),
      ...(hasIssue !== undefined && { hasIssue: Boolean(hasIssue) }),
