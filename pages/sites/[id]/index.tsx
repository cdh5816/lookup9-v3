...
const SalesPanel = ({ sales }: { siteId: string; sales: any[]; canManage: boolean; onMutate: () => void }) => (
  <SimpleRecordsPanel
    title="영업 기록"
    emptyText="등록된 영업 기록이 없습니다."
    items={sales}
    columns={[
      { key: 'status', label: '상태' },
      { key: 'estimateAmount', label: '예상금액', render: (item: any) => formatNumber(item.estimateAmount) },
      { key: 'meetingNotes', label: '미팅 메모' },
      { key: 'createdBy', label: '작성자', render: (item: any) => item?.createdBy?.position ? `${item.createdBy.position} ${item.createdBy.name}` : item?.createdBy?.name || '-' },
      { key: 'createdAt', label: '등록일', render: (item: any) => formatDate(item.createdAt) },
    ]}
  />
);

const ContractPanel = ({ contracts }: { siteId: string; contracts: any[]; canManage: boolean; onMutate: () => void }) => (
  <SimpleRecordsPanel
    title="수주 / 계약"
    emptyText="등록된 계약 기록이 없습니다."
    items={contracts}
    columns={[
      { key: 'status', label: '상태' },
      { key: 'contractAmount', label: '계약금액', render: (item: any) => formatNumber(item.contractAmount) },
...
const PaintPanel = ({ specs }: { siteId: string; specs: any[]; canManage: boolean; onMutate: () => void }) => (
  <SimpleRecordsPanel
    title="도장 사양"
    emptyText="등록된 도장 사양이 없습니다."
    items={specs}
    columns={[
      { key: 'colorCode', label: '컬러코드' },
      { key: 'colorName', label: '컬러명' },
      { key: 'manufacturer', label: '제조사' },
      { key: 'status', label: '상태' },
      { key: 'quantity', label: '수량', render: (item: any) => formatNumber(item.quantity) },
      { key: 'confirmedBy', label: '확인자', render: (item: any) => item?.confirmedBy?.name || '-' },
    ]}
  />
);
        ...
        <table className="table w-full">
          ...
          <tbody>
            {items.map((item: any) => (
              <tr key={item.id}>
                {columns.map((column) => (
                  <td key={column.key} className="text-sm whitespace-pre-wrap">
                    {column.render ? column.render(item) : displayValue(item?.[column.key])}
                  </td>
                ))}
              </tr>
            ))}
