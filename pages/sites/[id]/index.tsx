...
const upsertLabeledValue = (text: string | null | undefined, label: string, value: string) => {
  ...
  if (pattern.test(base)) return base.replace(pattern, (_m, prefix) => `${prefix}${line}`);
...
const getShipmentQuantity = (shipments: any[] = []) => shipments.reduce((sum, item) => sum + parseNumberValue(String(item?.quantity ?? item?.qty ?? item?.amount ?? item?.shippedQuantity ?? 0)), 0);

const ProgressBar = ({ value }: { value: number }) => (
  <div className="h-2 w-full overflow-hidden rounded-full bg-gray-800">
    <div className="h-full rounded-full bg-blue-600 transition-all" style={{ width: `${clampPercent(value)}%` }} />
  </div>
);
              ...
              <button type="button" onClick={() => setShowProgressDetails((prev) => !prev)} className="w-full text-left">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-2xl font-bold text-white">{finalProgress}%</p>
                  </div>
                  <div className="text-right text-xs text-gray-400">
                    <p>클릭해서 세부 공정률 보기</p>
                    <p className="mt-1">{showProgressDetails ? '접기' : '펼치기'}</p>
                  </div>
                </div>
              ...
              {showProgressDetails && (
                <div className="mt-4 border-t border-gray-800 pt-4">
                  <ProgressDetailEditor
                    ...
                    canManage={canManage}
                    panelRate={panelRate}
                    panelInQuantity={panelInQuantity}
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <div className="rounded-lg bg-black/20 p-3">
          <div className="mb-2 flex items-center justify-between text-xs text-gray-400">
            <span>하지파이프 진행률</span>
            <span>{clampPercent(pipeRate)}%</span>
          </div>
          <ProgressBar value={pipeRate} />
          {canManage ? (
            <input type="range" min="0" max="100" step="1" value={pipeRate} onChange={(e) => setPipeRate(Number(e.target.value))} className="range range-primary range-xs mt-3" />
          ) : null}
        </div>
        ...
        <div className="rounded-lg bg-black/20 p-3">
          ...
          <ProgressBar value={panelRate} />
          <p className="mt-2 text-xs text-gray-500">입고 {panelInQuantity.toLocaleString()} / 계약물량 {contractQuantity.toLocaleString()}</p>
        </div>

        <div className="rounded-lg bg-black/20 p-3">
          <div className="mb-2 flex items-center justify-between text-xs text-gray-400">
            <span>코킹작업 진행률</span>
            <span>{clampPercent(caulkingRate)}%</span>
          </div>
          <ProgressBar value={caulkingRate} />
          {canManage ? (
            <input type="range" min="0" max="100" step="1" value={caulkingRate} onChange={(e) => setCaulkingRate(Number(e.target.value))} className="range range-primary range-xs mt-3" />
          ) : null}
        </div>
      </div>

      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <label className="flex items-center justify-between rounded-lg border border-gray-700 bg-black/20 px-4 py-3 text-sm">
          <span>착수서류 완료</span>
          <input type="checkbox" className="checkbox checkbox-sm checkbox-primary" checked={startDocsDone} onChange={(e) => setStartDocsDone(e.target.checked)} disabled={!canManage} />
        </label>
        <label className="flex items-center justify-between rounded-lg border border-gray-700 bg-black/20 px-4 py-3 text-sm">
          <span>준공서류 완료</span>
          <input type="checkbox" className="checkbox checkbox-sm checkbox-primary" checked={completionDocsDone} onChange={(e) => setCompletionDocsDone(e.target.checked)} disabled={!canManage} />
        </label>
      </div>

      {canManage ? (
        <div className="flex justify-end">
          <Button size="sm" color="primary" loading={savingProgress} onClick={handleSaveProgress}>세부공정률 저장</Button>
        </div>
      ) : null}
    </div>
  );
